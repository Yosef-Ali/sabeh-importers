...
export default async function CategoryPage({ params }: CategoryPageProps) {
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Hero Section */}
      <section className="relative h-[400px] overflow-hidden bg-gradient-to-br from-navy via-navy-light to-navy">
        {/* Background Pattern */}
        ...
        <div className="container relative z-10 flex h-full flex-col justify-center">
          <div className="max-w-3xl">
            ...
            <div className="space-y-4">
              <Badge className="bg-gold/20 text-gold border-gold/30 uppercase tracking-widest text-xs font-bold">
                {category.name}
              </Badge>
            ...
            <div className="mt-8">
              <Link href={`/search?categoryId=${category.id}`}>
                <Button className="bg-gold hover:bg-gold/90 text-navy font-bold uppercase tracking-wide px-8 py-6 text-sm shadow-xl">
      ...
      <section className="container py-16">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-bold text-foreground">Latest {category.name}</h2>
            <p className="text-muted-foreground mt-1">Fresh arrivals from verified sellers</p>
          </div>
          {total > 12 && (
            <Link href={`/search?categoryId=${category.id}`}>
        ...
        {listings.length === 0 ? (
          <div className="flex min-h-[300px] items-center justify-center rounded-xl border border-dashed border-border bg-card p-12">
            <div className="text-center">
              <p className="text-lg text-muted-foreground">No listings found in this category yet.</p>
            ...
            {categories
              ...
              .map((cat) => (
                <Link
                  ...
                  className="group flex flex-col items-center rounded-xl border border-border bg-muted p-6 transition-all hover:border-gold hover:shadow-lg"
                >
                  <div className="mb-3 flex h-16 w-16 items-center justify-center rounded-full bg-card text-3xl shadow-sm">
                    {cat.icon || "📦"}
                  </div>
                  <p className="text-center text-sm font-bold text-foreground group-hover:text-gold transition-colors">
                    {cat.name}
                  </p>
                </Link>
              ))}
